...
`r Biocpkg("ASSIGN")` (Adaptive Signature Selection and InteGratioN) for
signature-based profiling of heterogeneous biological pathways.
`r Biocpkg("ASSIGN")` is a computational tool used to evaluate the pathway
deregulation/activation status in individual patient samples.
`r Biocpkg("ASSIGN")` employs a flexible Bayesian factor analysis approach that
...
Some distinctive features of `r Biocpkg("ASSIGN")` are:

1. __Multiple Pathway Profiling__: `r Biocpkg("ASSIGN")` can profile multiple
...
platforms. `r Biocpkg("ASSIGN")` can adaptively estimate background gene
expression levels across a set of samples.
3. __Context-specific signature estimation__: `r Biocpkg("ASSIGN")` provides the
...
4. __Regularization of signature strength estimates__: `r Biocpkg("ASSIGN")`
...
In the following examples, we will illustrate how to run `r Biocpkg("ASSIGN")`
using either the easy to use `assign.wrapper` function for simple
analysis or each individual `r Biocpkg("ASSIGN")` step for more detailed
intermediate results.

For either analysis, we will first load `r Biocpkg("ASSIGN")` and create a
...
are optional. `r Biocpkg("ASSIGN")` outputs additional validation plots to
...
## Run `r Biocpkg("ASSIGN")` all-in-one using `assign.wrapper`

We developed an all-in-one `assign.wrapper` function to run
`r Biocpkg("ASSIGN")` with one command. For most users, `assign.wrapper` will be
sufficient. The `assign.wrapper` function outputs the following files:

* __pathway_activity_testset.csv__: `r Biocpkg("ASSIGN")` predicted
...
sections below and the `r Biocpkg("ASSIGN")` reference manual.
...
## Run `r Biocpkg("ASSIGN")` step-by-step
...
The `r Biocpkg("ASSIGN")` package allows a signature to be adapted to fit other
...
gene in a cell line, but during the adaptation step, `r Biocpkg("ASSIGN")`
reduces the importance of that key gene, the quality of the
`r Biocpkg("ASSIGN")` predictions may be reduced. Alternatively, if a gene in
the signature is associated with some other heterogeneity in the data, such as
smoking status, `r Biocpkg("ASSIGN")` may adapt to differences in that gene,
...
signature, and genes to exclude from the signature. `r Biocpkg("ASSIGN")`
...
By default, `r Biocpkg("ASSIGN")` Bayesian gene selection chooses the signature
...
When running `r Biocpkg("ASSIGN")`, the number of genes in the gene list can
affect the predictions that `r Biocpkg("ASSIGN")` produces, but it is not always
clear how long the gene list should be. Included within `r Biocpkg("ASSIGN")` is
...
This function runs `r Biocpkg("ASSIGN")` pathway prediction on various gene list
lengths to find the optimum gene list length for the GFRN pathways by
correlating the `r Biocpkg("ASSIGN")` predictions to a matrix of correlation
data that you provide. This function takes a long time to run because you are
running `r Biocpkg("ASSIGN")` many times on many pathways, so I recommend
parallelizing by pathway or running the `r Biocpkg("ASSIGN")` predictions first
...
correlating `r Biocpkg("ASSIGN")` predictions with proteomics data. First, read
in the test data that you want to predict using `r Biocpkg("ASSIGN")` and the
...
`r Biocpkg("ASSIGN")` will output the results for each gene list length in the
...
# Citing `r Biocpkg("ASSIGN")`

If you use `r Biocpkg("ASSIGN")` in your publication, please cite:
...
Please see the `r Biocpkg("ASSIGN")` reference manual for full descriptions of
